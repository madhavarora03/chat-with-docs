...
ENV_LOG_FILE_ENABLED = "LOG_FILE_ENABLED"
ENV_LOG_FILE_PATH = "LOG_FILE_PATH"
ENV_LOG_CONFIG_PATH = "LOG_CONFIG_PATH"
...
def _apply_environment_overrides(config: dict) -> dict:
    ...
    file_enabled = os.getenv(ENV_LOG_FILE_ENABLED, "false").lower() == "true"
    
    if file_enabled:
        # Ensure logs directory exists
        log_file_path = os.getenv(ENV_LOG_FILE_PATH, DEFAULT_LOG_FILE_PATH)
        ...
        for logger_key in ["root", "loggers"]:
            if logger_key == "root" and "root" in config:
                if "file" not in config["root"].get("handlers", []):
                    config["root"]["handlers"] = config["root"].get("handlers", []) + ["file"]
                ...
                for logger_name in ["app"]:
                    if logger_name in config["loggers"]:
                        ...
                        if "file" not in handlers:
                            config["loggers"][logger_name]["handlers"] = handlers + ["file"]
    else:
...
def setup_logging(config_path: Optional[str] = None) -> None:
    ...
    Args:
        config_path: Optional path to the logging config file.
                    Defaults to LOG_CONFIG_PATH env var or 'logging.yaml'
    ...
    if config_path is None:
        config_path = os.getenv(ENV_LOG_CONFIG_PATH, DEFAULT_CONFIG_PATH)
